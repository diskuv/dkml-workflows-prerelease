...
# autogen from pc_matrix. only linux_x86_64{% for outer in pc_matrix %}{%- if outer.dkml_host_abi == "linux_x86_64" -%}{{ nl }}{% for var in outer.vars %}export {{ var.name }}="{{ var.value }}"{{ nl }}{% endfor %}{%- endif %}{% endfor %}
