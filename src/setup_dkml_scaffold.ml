...
(action
  ...
  OCAMLRUNPARAM
  b
  (run %%{bin:gh-setup-dkml-action-yml} --output-%s %%{target}))))
...
(action
  ...
  OCAMLRUNPARAM
  b
  (run %%{bin:pc-setup-dkml} --output-windows_x86 %%{target}))))
...
(action
  ...
  OCAMLRUNPARAM
  b
  (run %%{bin:pc-setup-dkml} --output-windows_x86_64 %%{target}))))
