...
export PIN_IOSTREAM_CAMLZIP='0.2.2'
...
export PIN_TINY_HTTPD_CAMLZIP='0.16'
...
export bootstrap_opam_version="2.2.0-beta2-20240409"
...
usage() {
  ...
  echo "  --PIN_CAMLZIP=<value>. Defaults to: ${PIN_CAMLZIP}" >&2
  ...
  echo "  --PIN_CONF_ZLIB=<value>. Defaults to: ${PIN_CONF_ZLIB}" >&2
  ...
  echo "  --PIN_IOSTREAM_CAMLZIP=<value>. Defaults to: ${PIN_IOSTREAM_CAMLZIP}" >&2
  ...
  echo "  --PIN_TINY_HTTPD_CAMLZIP=<value>. Defaults to: ${PIN_TINY_HTTPD_CAMLZIP}" >&2
...
while getopts :h-: option; do
  ...
  -) case $OPTARG in
    ...
    PIN_CAMLZIP) fail "Option \"$OPTARG\" missing argument" ;;
    PIN_CAMLZIP=*) PIN_CAMLZIP=${OPTARG#*=} ;;
    ...
    PIN_CONF_LIBX11=*) PIN_CONF_LIBX11=${OPTARG#*=} ;;
    ...
    PIN_CONF_ZLIB) fail "Option \"$OPTARG\" missing argument" ;;
    PIN_CONF_ZLIB=*) PIN_CONF_ZLIB=${OPTARG#*=} ;;
    ...
    PIN_IOSTREAM_CAMLZIP) fail "Option \"$OPTARG\" missing argument" ;;
    PIN_IOSTREAM_CAMLZIP=*) PIN_IOSTREAM_CAMLZIP=${OPTARG#*=} ;;
    ...
    PIN_TINY_HTTPD_CAMLZIP) fail "Option \"$OPTARG\" missing argument" ;;
    PIN_TINY_HTTPD_CAMLZIP=*) PIN_TINY_HTTPD_CAMLZIP=${OPTARG#*=} ;;
...
do_pins() {
    ...
    opamrun pin add --switch "$do_pins_NAME"  --yes --no-action -k version camlzip "${PIN_CAMLZIP}"
    ...
    opamrun pin add --switch "$do_pins_NAME"  --yes --no-action -k version conf-c++ "${PIN_CONF_C++}"
    opamrun pin add --switch "$do_pins_NAME"  --yes --no-action -k version conf-dkml-sys-opam "${PIN_CONF_DKML_SYS_OPAM}"
    opamrun pin add --switch "$do_pins_NAME"  --yes --no-action -k version conf-libX11 "${PIN_CONF_LIBX11}"
    ...
    opamrun pin add --switch "$do_pins_NAME"  --yes --no-action -k version conf-zlib "${PIN_CONF_ZLIB}"
    ...
    opamrun pin add --switch "$do_pins_NAME"  --yes --no-action -k version iostream-camlzip "${PIN_IOSTREAM_CAMLZIP}"
    ...
    opamrun pin add --switch "$do_pins_NAME"  --yes --no-action -k version tiny_httpd_camlzip "${PIN_TINY_HTTPD_CAMLZIP}"
